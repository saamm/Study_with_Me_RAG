...
# Setup the conversational retrieval chain based on selected chapter and subject
# Loads vector DB for either full subject or chapter.
# Loads embeddings to perform similarity search.
# Loads LLaMA 70B model via Groq to generate high-quality answers.
# Enables conversation memory, so follow-up questions work.
# Creates a Conversational Retrieval Chain, which:
# Retrieves the top 3 most relevant text chunks using MMR.
# Sends them to the LLM.
# Generates an answer based only on retrieved content.
# Returns the retrieved documents for transparency.
