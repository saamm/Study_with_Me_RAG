...
# if user wants to ask question from all chapters then this vector db will be used
# vector_db_dir is where vector DBs will be stored
vector_db_dir = f"{parent_dir}/vector_db"

# if user wants to ask question chapter-wise then separate vector DBs will be created for each chapter
# chapters_vector_db_dir is where chapter-wise vector DBs will be stored
chapters_vector_db_dir = f"{parent_dir}/chapters_vector_db"

# Initialize embedding model and text splitter
embedding = HuggingFaceEmbeddings()

# text splitter to split documents into chunks of size 2000 with overlap of 500 characters
text_splitter = CharacterTextSplitter(chunk_size=2000, chunk_overlap=500)

# Function to vectorize the entire book and store it in a vector database
def vectorize_book_and_store_to_db(class_subject_name, vector_db_name):
    ...
    # Directory loader to load all PDF files in the book directory
...
# Function to vectorize each chapter separately and store in chapter-wise vector databases
